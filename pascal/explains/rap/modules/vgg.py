...
import torch
from explain.rap.modules.layers import *
...
class VGG_VOC(VGG):
    def __init__(self, reduce, features, classifier, init_weights=True):
        super(VGG, self).__init__()
        self.features = features
        self.avgpool = AdaptiveAvgPool2d((7, 7))
        channels = int(512/reduce)
        if classifier == None:
            self.classifier = Sequential(
                Dropout(),
                Linear(channels * 7 * 7, 20))
        else:
            self.classifier = classifier
        if init_weights:
            self._initialize_weights()

def vgg16_bn_voc(config=None, reduce=1, pretrained=False, **kwargs):
    ...
    if config is None:
        config = cfg['D']
        if reduce == 1:
            model = VGG_VOC(reduce, make_layers(cfg['D'],batch_norm=True), classifier=None, **kwargs)
        elif reduce == 2:
            model = VGG_VOC(reduce, make_layers(cfg['Dx2'],batch_norm=True), classifier=None, **kwargs)
        elif reduce == 4:
            model = VGG_VOC(reduce, make_layers(cfg['Dx4'], batch_norm=True), classifier=None, **kwargs)
        elif reduce == 8:
            model = VGG_VOC(reduce, make_layers(cfg['Dx8'], batch_norm=True), classifier=None, **kwargs) 
    else:
        features, classifier = _make_layers_rap(config, input_dims=(3, 227, 227))
        model = VGG_VOC(reduce, features, classifier, **kwargs)        
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['vgg16_bn']))
    return model

def _make_layers_rap(cfg, input_dims):
    layers = []
    ...
    for x in cfg['classifier']:
        if x == 'R':
            ...
            layers += [Linear(linear_input_dims, x)]
            linear_input_dims = x
...
cfg = {
    ...
    'Dx2': [32, 32, 'M', 64, 64, 'M', 128, 128, 128, 'M', 256, 256, 256, 'M', 256, 256, 256, 'M'],
    'Dx4': [16, 16, 'M', 32, 32, 'M', 64, 64, 64, 'M', 128, 128, 128, 'M', 128, 128, 128, 'M'],
    'Dx8': [8, 8, 'M', 16, 16, 'M', 32, 32, 32, 'M', 64, 64, 64, 'M', 64, 64, 64, 'M'],
